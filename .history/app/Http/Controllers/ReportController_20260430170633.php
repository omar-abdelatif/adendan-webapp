...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $sumTotalSubscriptionDues = (int) Due::where('transaction_type', 'إشتراك')->where('item', '!=', $currentSubscriptionYear)->sum('amount_remaining');
        $sumDueDonationAmount = (int) Due::where('transaction_type', 'تبرعات')->sum('total_amount');
        $subscriptionSumDuePaied = (int) Due::where('transaction_type', 'إشتراك')->sum('amount_paid');
        $sumTotalCurrentSubscriptionDues = (int) Due::where('transaction_type', 'إشتراك')->whereYear('item', $currentSubscriptionYear)->sum('total_amount');
        $sumTotalCurrentSubscriptionDuesPaied = (int) PaymentTransaction::where('transaction_type', 'اشتراك')->whereYear('item', $currentSubscriptionYear)->sum('amount');
        dd($sumTotalCurrentSubscriptionDues);
        return view('pages.reports.subscriptions', compact('subscribers', 'subscriptions', 'count', 'sumTotalSubscriptionDues', 'sumDueDonationAmount', 'subscriptionSumDuePaied', 'currentSubscriptionYear', 'sumTotalCurrentSubscriptionDues', 'sumTotalCurrentSubscriptionDuesPaied'));
    }
    ...
    public function search(Request $request)
    {
        ...
        $query = Subscribers::select('subscribers.name', 'subscribers.member_id')->join('dues', 'subscribers.member_id', '=', 'dues.member_id')->where('dues.transaction_type', 'إشتراك')->selectRaw('dues.total_amount, dues.amount_paid, dues.amount_remaining');
        if (empty($amount)) {
